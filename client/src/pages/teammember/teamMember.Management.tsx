...
const fetchTeamMembers = async ({
    ...
    const response = await apiRequest(
        "GET",
        `/user/team-member/list?pageNo=${page}&pagesize=${pageSize}&pagination_required=true`
    );
...
const TeamMemberList: React.FC = () => {
    ...
    const handleNextPage = () => {
        if (teamMembersResponse && page < teamMembersResponse.pagination.totalPages) {
            setPage((p) => p + 1);
        }
    };
                                        ...
                                        {teamMembersResponse.teamMembers.map((member) => (
                                            ...
                                            >
                                                <TableCell className="text-gray-600">
                                                    {member.members_serial_no}
                                                </TableCell>
                                                <TableCell>
                                                    <Avatar className="h-10 w-10">
                                                        <AvatarImage src={member.profile_url} alt={member.first_name} />
                                                ...
                                                <TableCell>
                                                    <span
                                                        className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${member.status === 1
                                                ...
                                                <TableCell className="text-right">
                                                    <div className="flex items-center justify-end gap-2">
                                                        <Link href={`/teammembers/view/${member.id}`}>
                                                            <Button
                                                                variant="ghost"
                                                                size="sm"
                                                        ...
                                                        <Link href={`/teammembers/edit/${member.id}`}>
                                                            <Button
                                                                variant="ghost"
                                                                size="sm"
                                                                className="hover:bg-teal-50 hover:text-teal-600"
                            ...
                            <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-100">
                                ...
                                <div className="flex items-center space-x-2">
                                    ...
                                    <span className="text-sm font-medium text-gray-700">
                                        Page {page} of {teamMembersResponse?.pagination.totalPages || 1}
                                    </span>
                                    <Button
                                        variant="outline"
                                        size="sm"
                                        ...
                                        disabled={
                                            !teamMembersResponse || page >= teamMembersResponse.pagination.totalPages
                                        }
                                        className="hover:bg-gray-50 hover:text-teal-600"
                                    >
